...
public class MarketController {
    ...
    public String BuyCard(Integer marketId, String token, Integer cardId,  Integer sellerId, HttpServletResponse response,HttpServletRequest request) {
		...
		UserDto buyer = sessionService.isLogged(token , request);
	 	UserDto seller = uRepository.findById(sellerId)
	 			.orElseThrow(() -> new RuntimeException("Pas d'utilisateur"));
	 	Card card = cRepository.findById(cardId).orElseThrow(() -> new RuntimeException("Pas de carte trouvée avec cet id"));
	...
	public String SellCard(String token,  Integer cardId, HttpServletResponse response,HttpServletRequest request) {
		...
		User seller = mService.getUserBySession(token , request);
